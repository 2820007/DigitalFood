...
exports.getMyOrders=async(req,res)=>{
    ...
    const orders=await Order.find({user:userId}).populate({
        path:"items.product",
        model:"Product",
        select:"-productStockQnt -createdAt"
    })
    if(orders.length ==0){
...
exports.updateMyOrder=async(req,res)=>{
    ...
    // check if the trying to update the order user is true of this order
    ...
    if(existingOrder.orderStatus ==="ontheway"){
        return res.status(400).json({
            message:"You can't update when it is on the way"
        })
    }
...
exports.cancelOrder=async(req,res)=>{
    ...
    if(order.orderStatus !=="pending"){
        return res.status(400).json({
            message:"You can't cancel this order as it is not pending."
        })
    }
